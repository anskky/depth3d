...
class DepthProEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> list[torch.Tensor]:
        ...
        # batch_size = 8
        # outputs = []
        # for i in range(0, x_pyramid_patches.size(0), batch_size):
        #     batch = x_pyramid_patches[i:i+batch_size]
        #     batch = batch.to(torch.device("cpu"))  # Ensure batch is on the correct device
        #     with torch.no_grad():  # Disable gradient calculation to save memory
        #         print('cycle')
        #         output = self.patch_encoder(batch)
        #     outputs.append(output)
        # x_pyramid_encodings = torch.cat(outputs, dim=0)
